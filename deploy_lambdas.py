...
def deploy_lambda(lambda_dir, repository_name, function_name):
    ...
    aws_region = "us-east-1"  # Hardcoded based on your configuration
    aws_account = run_command("aws sts get-caller-identity --query Account --output text")
...
def main():
    # Configure for your specific Lambda setup with correct paths
    lambdas = [
        ('src/lambda2', 'trading-lambda-lookup', 'trading-prod-symbol-lookup'),  # Container-based lambda
    ]
